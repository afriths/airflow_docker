...
// DAG list selectors
export const selectDAGsList = createSelector(
  [selectDAGs],
  (dags) => dags.items
);
...
export const selectDAGsError = createSelector(
  [selectDAGs],
  (dags) => dags.error
);
...
export const selectDAGById = (dagId: string) => createSelector(
  [selectDAGsList],
  (dagsList) => dagsList.find(dag => dag.dag_id === dagId) || null
);

// Filtered DAGs selectors
export const selectActiveDAGs = createSelector(
  [selectDAGsList],
  (dagsList) => dagsList.filter(dag => !dag.is_paused)
);

export const selectPausedDAGs = createSelector(
  [selectDAGsList],
  (dagsList) => dagsList.filter(dag => dag.is_paused)
);

export const selectDAGsWithErrors = createSelector(
  [selectDAGsList],
  (dagsList) => dagsList.filter(dag => dag.has_import_errors)
);

// DAGs with running status
export const selectRunningDAGs = createSelector(
  [selectDAGsList],
  (dagsList) => dagsList.filter(dag => dag.last_run_state === 'running')
);

export const selectFailedDAGs = createSelector(
  [selectDAGsList],
  (dagsList) => dagsList.filter(dag => dag.last_run_state === 'failed')
);

export const selectSuccessfulDAGs = createSelector(
  [selectDAGsList],
  (dagsList) => dagsList.filter(dag => dag.last_run_state === 'success')
);

// DAG statistics
export const selectDAGStats = createSelector(
  [selectDAGsList],
  (dagsList) => {
    const stats = {
...
export const selectDAGsByOwner = createSelector(
  [selectDAGsList],
  (dagsList) => {
...
export const selectDAGsByTag = createSelector(
  [selectDAGsList],
  (dagsList) => {
...
export const selectFilteredDAGs = (searchTerm: string, filters: {
  paused?: boolean;
  tags?: string[];
  owners?: string[];
  states?: string[];
    ...
    return dagsList.filter(dag => {
      ...
      if (searchTerm) {
        ...
        const matchesSearch = 
          ...
          (dag.description && dag.description.toLowerCase().includes(searchLower)) ||
      ...
      if (filters.owners && filters.owners.length > 0) {
        const hasMatchingOwner = filters.owners.some(owner => dag.owners.includes(owner));
        if (!hasMatchingOwner) return false;
      }

      // States filter
      if (filters.states && filters.states.length > 0) {
        if (!dag.last_run_state || !filters.states.includes(dag.last_run_state)) {
          return false;
        }
      }
