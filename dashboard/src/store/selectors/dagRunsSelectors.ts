...
export const selectDAGRunsForDAG = (dagId: string) => createSelector(
  [selectDAGRuns],
  (dagRuns) => dagRuns[dagId] || {
    runs: [],
    loading: false,
    error: null,
    lastUpdated: null,
...
export const selectDAGRunsList = (dagId: string) => createSelector(
  [selectDAGRunsForDAG(dagId)],
  (dagRunsState) => dagRunsState.runs
);

// DAG runs loading state for specific DAG
export const selectDAGRunsLoading = (dagId: string) => createSelector(
  [selectDAGRunsForDAG(dagId)],
  (dagRunsState) => dagRunsState.loading
);

// DAG runs error for specific DAG
export const selectDAGRunsError = (dagId: string) => createSelector(
  [selectDAGRunsForDAG(dagId)],
  (dagRunsState) => dagRunsState.error
);

// Selected DAG run ID for specific DAG
export const selectSelectedDAGRunId = (dagId: string) => createSelector(
  [selectDAGRunsForDAG(dagId)],
  (dagRunsState) => dagRunsState.selectedRun
);

// Selected DAG run for specific DAG
export const selectSelectedDAGRun = (dagId: string) => createSelector(
...
export const selectLatestDAGRun = (dagId: string) => createSelector(
...
export const selectDAGRunById = (dagId: string, runId: string) => createSelector(
  [selectDAGRunsList(dagId)],
  (runs) => runs.find(run => run.dag_run_id === runId) || null
);

// Filtered DAG runs selectors
export const selectRunningDAGRuns = (dagId: string) => createSelector(
  [selectDAGRunsList(dagId)],
  (runs) => runs.filter(run => run.state === 'running')
);

export const selectFailedDAGRuns = (dagId: string) => createSelector(
  [selectDAGRunsList(dagId)],
  (runs) => runs.filter(run => run.state === 'failed')
);

export const selectSuccessfulDAGRuns = (dagId: string) => createSelector(
  [selectDAGRunsList(dagId)],
  (runs) => runs.filter(run => run.state === 'success')
);

export const selectManualDAGRuns = (dagId: string) => createSelector(
  [selectDAGRunsList(dagId)],
  (runs) => runs.filter(run => run.run_type === 'manual')
);

export const selectScheduledDAGRuns = (dagId: string) => createSelector(
  [selectDAGRunsList(dagId)],
  (runs) => runs.filter(run => run.run_type === 'scheduled')
);

// DAG run statistics for specific DAG
export const selectDAGRunStats = (dagId: string) => createSelector(
  [selectDAGRunsList(dagId)],
  (runs) => {
    const stats = {
      total: runs.length,
...
export const selectRecentDAGRuns = (dagId: string) => createSelector(
  [selectDAGRunsList(dagId)],
  (runs) => runs.slice(0, 10)
);

// DAG runs with duration calculated
export const selectDAGRunsWithDuration = (dagId: string) => createSelector(
  [selectDAGRunsList(dagId)],
  (runs) => runs.map(run => ({
    ...run,
...
export const selectAllDAGRuns = createSelector(
  [selectDAGRuns],
  (dagRuns) => {
...
export const selectDAGRunsDataAge = (dagId: string) => createSelector(
  [selectDAGRunsForDAG(dagId)],
  (dagRunsState) => {
...
export const selectDAGRunsNeedRefresh = (dagId: string) => createSelector(
  [selectDAGRunsDataAge(dagId)],
  (dataAge) => {
